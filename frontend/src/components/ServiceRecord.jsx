...
export default function ServiceRecord({ service }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg border-2 border-border p-6">
      {/* Header */}
      <div className="border-b-2 border-border pb-4 mb-6">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Service Record</h2>
            <p className="text-sm text-muted-foreground mt-1">BERHANU AL-ADEM Auto Solution</p>
          </div>
          {service.reference_number && (
            <div className="text-right">
              <p className="text-sm text-muted-foreground">REF No.</p>
              <p className="text-lg font-bold text-foreground">{service.reference_number}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Service Details */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6 p-4 bg-muted/35 rounded-lg">
        <div>
        ...
        <div>
          <p className="text-sm text-muted-foreground mb-1">Kilometer</p>
          <p className="font-semibold text-foreground">
        ...
        {service.oil_type && (
          <div>
            <p className="text-sm text-muted-foreground mb-1">Oil Type</p>
            <p className="font-semibold text-foreground">{service.oil_type}</p>
          </div>
        )}
        <div>
          <p className="text-sm text-muted-foreground mb-1">Next Service KM</p>
          <p className="font-semibold text-foreground">
        ...
        {service.service_note && (
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="font-semibold text-foreground">{service.service_note}</p>
          </div>
        )}
      </div>
      ...
      {service.checklist_items && service.checklist_items.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-bold text-foreground mb-4">Service Checklist</h3>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-muted">
                  <th className="border border-border px-4 py-2 text-left font-semibold text-foreground/90">
                    Item
                  </th>
                  <th className="border border-border px-4 py-2 text-center font-semibold text-foreground/90 w-24">
                    CHECKED
                  </th>
                  <th className="border border-border px-4 py-2 text-center font-semibold text-foreground/90 w-24">
                    CHANGED
                  ...
                  <tr key={item.checklist_id} className="hover:bg-muted/45">
                    ...
                    <td className="border border-border px-4 py-2 text-center">
                      {item.checked ? (
                        ...
                        <XCircle className="mx-auto text-muted-foreground/35" size={20} />
                      )}
                    </td>
                    <td className="border border-border px-4 py-2 text-center">
                      {item.changed ? (
      ...
      {service.parts && service.parts.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-bold text-foreground mb-4">Parts Used</h3>
            ...
            {service.parts.map((part, index) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <p className="font-semibold text-foreground">{part.part_name}</p>
      ...
      {/* Cost Breakdown */}
      <div className="mb-6 p-4 bg-muted/35 rounded-lg">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between border-t-2 border-border pt-2 mt-2 font-bold text-lg">
            <span>Total:</span>
      ...
      <div className="border-t-2 border-border pt-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          {service.serviced_by_name && (
            <div>
              <p className="text-muted-foreground mb-1">Serviced By</p>
              <p className="font-semibold text-foreground">{service.serviced_by_name}</p>
            </div>
          )}
          {service.branch && (
            <div>
              <p className="text-muted-foreground mb-1">Branch</p>
              <p className="font-semibold text-foreground">{service.branch}</p>
            </div>
          )}
          <div>
          ...
          <div>
            <p className="text-muted-foreground mb-1">Cost/Amount</p>
            <p className="font-semibold text-foreground">
