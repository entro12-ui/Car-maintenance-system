...
export default function ManageServiceChecklists() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-foreground mb-6">Manage Service Checklists</h1>
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-lg border transition-colors ${
                    selectedServiceType === st.service_type_id
                      ...
                      : 'bg-muted/35 border-border hover:bg-muted/70'
                  }`}
                >
                  <div className="font-semibold">{st.type_name}</div>
                  <div className={`text-sm ${selectedServiceType === st.service_type_id ? 'text-primary-100' : 'text-muted-foreground'}`}>
                    {st.description}
                  </div>
                </button>
              ))}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div className="flex items-end">
                        <label className="flex items-center space-x-2 cursor-pointer">
                          ...
                          />
                          <span className="text-sm text-foreground/90">Mandatory</span>
                        </label>
                      </div>
                    </div>
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        className="flex items-center space-x-2 rounded-xl border border-border bg-muted px-4 py-2 font-medium text-foreground/90 shadow-sm transition hover:bg-muted/80"
                      >
                        <X size={16} />
                        <span>Cancel</span>
                      </button>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-muted">
                      <th className="border border-border px-4 py-2 text-left font-semibold text-foreground/90">Sort</th>
                      <th className="border border-border px-4 py-2 text-left font-semibold text-foreground/90">Item Name</th>
                      <th className="border border-border px-4 py-2 text-left font-semibold text-foreground/90">Description</th>
                      <th className="border border-border px-4 py-2 text-center font-semibold text-foreground/90">Mandatory</th>
                      <th className="border border-border px-4 py-2 text-center font-semibold text-foreground/90">Duration</th>
                      <th className="border border-border px-4 py-2 text-center font-semibold text-foreground/90">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      checklistItems.map((item) => (
                        <tr key={item.checklist_id} className="hover:bg-muted/45">
                          {editingItem?.checklist_id === item.checklist_id ? (
                            ...
                            <>
                              <td className="border border-border px-4 py-2 text-center">{item.sort_order}</td>
                              <td className="border border-border px-4 py-2 font-medium">{item.item_name}</td>
                              ...
                              <td className="border border-border px-4 py-2 text-center">{item.estimated_duration_minutes} min</td>
                              <td className="border border-border px-4 py-2">
                                <div className="flex justify-center space-x-2">
                                  <button
