...
export default function CustomerVehicles() {
  ...
  const { data: vehicles, isLoading } = useQuery({
    ...
    refetchInterval: 30000, // Auto-refresh every 30 seconds to catch admin-added vehicles
  })

  const createVehicleMutation = useMutation({
    mutationFn: (data) => customerApi.createVehicle(data),
    onSuccess: () => {
      setSuccessMessage('Vehicle added successfully!')
      queryClient.invalidateQueries(['customer-vehicles'])
      queryClient.invalidateQueries(['customer-summary'])
    ...
    onError: (error) => {
      ...
      if (error.response?.data?.detail?.includes('License plate')) {
        setErrors({ license_plate: 'License plate already registered' })
      } else if (error.response?.data?.detail?.includes('VIN')) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {vehicles.map((vehicle) => (
            <div
              ...
              className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow border-l-4 border-primary-500"
            >
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t mt-6">
                ...
                >
                  {createVehicleMutation.isLoading ? 'Adding...' : 'Add Vehicle'}
                </button>
              </div>
            </form>
          </div>
